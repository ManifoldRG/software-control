...
class CalcTools:
    ...
    def env_info(cls, sheet_name=None):
        ...
        try:
            ...
            header_row = "  | " + " | ".join(f"{h:<{w}}" for h, w in zip(col_headers, col_widths, strict=False)) + " |"
            ...
            for row_idx, row in enumerate(data_array, 1):
                row_str = (
                    f"{row_idx:<2}| " + " | ".join(f"{cell:<{w}}" for cell, w in zip(row, col_widths, strict=False)) + " |"
                )
    ...
    def transpose_range(cls, source_range, target_cell):
        ...
        try:
            ...
            # 转置数据
            transposed_data = list(map(list, zip(*data, strict=False)))
    ...
    def set_number_format(cls, range_str, format_type, decimal_places=None):
        ...
        try:
            ...
            if format_type == "general":
                ...
                if decimal_places is not None:
                    format_string = (
                        f"[$¥-804]#,##0{('.' + '0' * decimal_places) if decimal_places > 0 else ''}"
                    )
                else:
