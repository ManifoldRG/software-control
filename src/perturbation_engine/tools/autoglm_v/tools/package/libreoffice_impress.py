    ...
    def go_to_slide(cls, slide_index):
        ...
        try:
            ...
            if zero_based_index < 0 or zero_based_index >= pages.getCount():
                cls.ret = (
                    f"Error: Slide index {slide_index} is out of range. Valid range is 1-{pages.getCount()}"
                )
                return False
    ...
    def duplicate_slide(cls, slide_index):
        ...
        try:
            ...
            if zero_based_index < 0 or zero_based_index >= draw_pages.getCount():
                cls.ret = (
                    f"Error: Invalid slide index {slide_index}. Valid range is 1 to {draw_pages.getCount()}"
                )
                return False
            controller = cls.doc.getCurrentController()
                                ...
                                if hasattr(para, "createEnumeration"):
                                    ...
                                    while para_enum.hasMoreElements():
                                        ...
                                        if (
                                            ...
                                            if (
                                                hasattr(portion, "TextField")
                                                and portion.TextField is not None
                                            ):
    ...
    def export_to_image(cls, file_path, format, slide_index=None):
        ...
        try:
            ...
            if slide_index is not None:
                ...
                if slide_index < 0 or slide_index >= page_count:
                    cls.ret = (
                        f"Error: Invalid slide index {slide_index + 1}. Valid range is 1 to {page_count}"
                    )
                    return False
            controller = cls.doc.getCurrentController()
            ...
            if slide_index is not None:
                ...
                if page_count == 1:
                    ...
                    cls.ret = (
                        f"Successfully exported {page_count} slides to {base_name}_[1-{page_count}].{format}"
                    )
                return True
        except Exception as e:
